...
const Shop = () => {
    ...
    const fetchProducts = (page, platform, minPrice, maxPrice, type, region, title, sort,categoryId) => {
        axios.get(`${Base_url}/products/getAll?page=${page}&platform=${platform}&minPrice=${minPrice}&maxPrice=${maxPrice}&type=${type}&region=${region}&title=${title}&sort=${sort}&categoryId=${categoryId}`)
            .then((res) => {
    ...
    const fetchCategories = () => {
        axios.get(`${Base_url}/category/getAll`).then((res) => {
